...
data = genfromtxt('Experiment5.csv', delimiter=',')
...
for epoch in range(int(train_len/10)):
	...
	model.fit(X, y,batch_size=1, verbose=1,validation_data=(X1,y1))
countp=0
countn=0
ypredicted = []
yactual = []
# evaluate LSTM
with codecs.open("output_exp3.txt", "a", "utf-8") as my_file:
...
Precision =precision_score(yactual, ypredicted)
Recall =recall_score(yactual, ypredicted)
print(classification_report(yactual, ypredicted))
print(accuracy_score(yactual,ypredicted))
...
plt.title('Fall or Not-Fall Confusion Matrix - Balanced 35 features')
